...
@section('content')
    ...
    <style>
        ...
        *::after {
        ...
        .btn-send {
            ...
            border: none;
            cursor: pointer;
            box-shadow: 0 4px 16px rgba(20, 85, 179, .28);
        ...
        .btn-login-nudge {
            ...
            box-shadow: 0 4px 14px rgba(20, 85, 179, .28);
            transition: opacity var(--ease), transform var(--ease);
        }

        .btn-login-nudge:hover {
            opacity: .9;
            transform: translateY(-1px);
            color: #fff;
        }
    ...
    <div class="fp-wrap">
        ...
        @if (session('warning'))
            <div class="fp-alert" style="background:#fff3cd;border-color:#ffeeba;color:#856404;">
        ...
        @if (session('error'))
            <div class="fp-alert" style="background:#ffebee;border-color:#ef9a9a;color:#c62828;">
        ...
        @auth
            @if (auth()->user()->status === 'banned')
                <div class="login-nudge">
                    <p>🚫 Akun kamu telah dibanned karena pelanggaran.</p>
                </div>
            @else
                <div class="cf">
                    ...
                    <form action="{{ route('forum.komentar.store', $forum->id) }}" method="POST">
                        @csrf
                        <textarea name="isi" placeholder="Bagikan pendapat...">{{ old('isi') }}</textarea>
        ...
        <div class="comment-list">
            @forelse($komentar as $komen)
                <div class="comment-card {{ $komen->user->warning_count >= 2 ? 'toxic-user' : '' }}">

                    {{-- top --}}
                    <div class="comment-top">
                        <div class="c-avatar">
                            @if ($komen->user->foto)
                                <img src="{{ asset('storage/' . $komen->user->foto) }}"
                                    style="width:100%;height:100%;object-fit:cover;border-radius:50%;">
                            @else
                                {{ strtoupper(substr($komen->user->name, 0, 1)) }}
                            @endif
                        </div>
                    ...
                    @else
                        {{-- ⚠️ LABEL KOMENTAR KASAR --}}
                        @if ($komen->is_kasar)
                            <div style="margin-bottom:8px;">
                                <span class="badge-banned" style="background:#ffebee;color:#c62828;">
                                    ⚠️ Komentar kasar
                                </span>
                            </div>
                        @endif
                        ...
                        @auth
                            @if (auth()->user()->hasRole('moderator'))
                                <div class="mod-zone">
                                    ...
                                    <form action="{{ route('moderator.komentar.destroy', $komen->id) }}" method="POST"
                                        style="display:inline;">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="btn-del">
                                    ...
                                    @if ($komen->user->status === 'active')
                                        <form action="{{ route('moderator.user.ban', $komen->user->id) }}" method="POST"
                                            style="display:inline;">
                                            @csrf
                                    ...
                                    @else
                                        <form action="{{ route('moderator.user.unban', $komen->user->id) }}" method="POST"
                                            style="display:inline;">
                                            @csrf
